...
def load_and_process_data(alias_data="", alias_insumos=""):
    ...
    if len(travel_times) > 0:
        ...
        etapas["travel_speed"] = (
            etapas["distance_osm_drive"] / (etapas["travel_time_min"] / 60)
        ).round(1)

        etapas["travel_time_min_tmp"] = etapas["travel_time_min"]
        etapas["travel_time_min_tmp"] = etapas["travel_time_min_tmp"].fillna(0)
        etapas["tmp"] = etapas.groupby(
            ["dia", "id_tarjeta", "id_viaje"]
        ).travel_time_min.transform(min)
        etapas.loc[etapas.tmp == 0, "travel_time_min_tmp"] = 0
        aggviajes = (
            etapas.groupby(["dia", "id_tarjeta", "id_viaje"], as_index=False)
            .travel_time_min_tmp.sum()
            .round(2)
            .rename(columns={"travel_time_min_tmp": "travel_time_min"})
        )
        etapas = etapas.drop(["travel_time_min_tmp", "tmp"], axis=1)

        viajes = viajes.merge(
            aggviajes, how="left", on=["dia", "id_tarjeta", "id_viaje"]
        )

        viajes["travel_speed"] = (
            viajes["distance_osm_drive"] / (viajes["travel_time_min"] / 60)
        ).round(1)

        viajes.loc[
            (viajes.travel_speed == np.inf) | (viajes.travel_speed >= 50),
            "travel_speed",
        ] = np.nan
    else:
    ...
    viajes_modo_agg = (
        etapas.groupby(["dia", "id_tarjeta", "id_viaje"])
        .apply(determinar_modo_agregado)
        .reset_index(name="modo_agregado")
        .sort_values(["dia", "id_tarjeta", "id_viaje"])
    ...
    etapas.loc[(etapas.hora >= 13) & (etapas.hora <= 16), "rango_hora"] = "13-16"
    etapas.loc[(etapas.hora >= 17) & (etapas.hora <= 24), "rango_hora"] = "17-24"
    ...
    viajes = viajes.merge(
        etapas.groupby(
            ["dia", "id_tarjeta", "id_viaje", "modo_agregado"], as_index=False
        )
        .size()
        .drop(["size"], axis=1),
        how="left",
    ...
    if len(etapas[etapas.distance_osm_drive.isna()]):
        print(
            f"Se van a borrar {len(etapas[etapas.distance_osm_drive.isna()])} registros sin datos de distancias en Etapas"
        )
        etapas = etapas[etapas.distance_osm_drive.notna()]
    if len(viajes[viajes.distance_osm_drive.isna()]):
        print(
            f"Se van a borrar {len(viajes[viajes.distance_osm_drive.isna()])} registros sin datos de distancias en Viajes"
        )
...
def construyo_indicadores(viajes, poligonos=False):
    ...
    ind1 = (
        ...
        .groupby(["id_polygon", "mes", "tipo_dia"], as_index=False)
        .Valor.mean()
        .round()
    )
    ...
    ind2 = ind2.merge(
        ind1[["id_polygon", "mes", "tipo_dia", "Valor"]].rename(
            columns={"Valor": "Tot"}
        ),
        how="left",
    )
    ...
    ind3 = (
        ...
        .round(0)
        .rename(columns={"factor_expansion_linea": "Valor"})
        .groupby(["id_polygon", "mes", "tipo_dia", "rango_hora"], as_index=False)
        .Valor.mean()
        .round()
    )
    ...
    ind4 = (
        ...
        .round(0)
        .rename(columns={"factor_expansion_linea": "Valor"})
        .groupby(["id_polygon", "mes", "tipo_dia", "modo"], as_index=False)
        .Valor.mean()
        .round()
    )
    ...
    ind9 = (
        viajes.groupby(
            ["id_polygon", "dia", "mes", "tipo_dia", "distancia"], as_index=False
        )
        .factor_expansion_linea.sum()
        .round(0)
        .rename(columns={"factor_expansion_linea": "Valor"})
        .groupby(["id_polygon", "mes", "tipo_dia", "distancia"], as_index=False)
        .Valor.mean()
        .round()
    )
    ...
    ind6 = (
        ...
        .rename(columns={"distance_osm_drive": "Valor"})
        .groupby(["id_polygon", "mes", "tipo_dia"], as_index=False)
        .Valor.mean()
        .round(2)
    )
    ...
    indicadores = indicadores[
        ["id_polygon", "mes", "tipo_dia", "Tipo", "Indicador", "Valor_str"]
    ].rename(columns={"Valor_str": "Valor"})

    indicadores = indicadores.sort_values(
        ["id_polygon", "mes", "tipo_dia", "Tipo", "Indicador"]
    )
...
def select_cases_from_polygons(etapas, viajes, polygons, res=8):
    ...
    for i, row in polygons.iterrows():
        ...
        gdf_hexs = select_h3_from_polygon(poly, res=res, viz=False)
        ...
        seleccionar = seleccionar.groupby(
            ["dia", "id_tarjeta", "id_viaje", "id_polygon"], as_index=False
        ).size()
        ...
        etapas_selec = pd.concat([etapas_selec, tmp], ignore_index=True)

        tmp = viajes.merge(seleccionar)
        viajes_selec = pd.concat([viajes_selec, tmp], ignore_index=True)

        gdf_hexs["polygon_lon"] = poly.representative_point().x.values[0]
        gdf_hexs["polygon_lat"] = poly.representative_point().y.values[0]

        gdf_hexs_all = pd.concat([gdf_hexs_all, gdf_hexs], ignore_index=True)
...
def creo_h3_equivalencias(polygons_h3, polygon, res, zonificaciones):
    ...
    if "res_" in res:
        ...
        if True:
            ...
            poly_ovl[f"lat_res_{resol}"] = (
                poly_ovl.geometry.to_crs(4326).representative_point().y
            )
            poly_ovl[f"lon_res_{resol}"] = (
                poly_ovl.geometry.to_crs(4326).representative_point().x
            )
        ...
        for zonas in zonificaciones.zona.unique():
            ...
            poly_ovl_agg[f"lat_{zonas}"] = (
                poly_ovl.geometry.to_crs(4326).representative_point().y
            )
            poly_ovl_agg[f"lon_{zonas}"] = (
                poly_ovl.geometry.to_crs(4326).representative_point().x
            )
            ...
            poly_ovl = poly_ovl.merge(
                poly_ovl_agg[["id", f"zona_{zonas}", f"lat_{zonas}", f"lon_{zonas}"]],
                on=f"id",
                how="left",
            )
            ...
            if len(poly_sel_all) == 0:
                ...
                poly_sel_all = poly_sel_all.merge(
                    poly_ovl[["h3_o", f"zona_{zonas}", f"lat_{zonas}", f"lon_{zonas}"]],
                    on="h3_o",
                    how="left",
                )
...
def crea_socio_indicadores(etapas, viajes):
    ...
    userx = (
        userx.groupby(["dia", "id_tarjeta"])["tarifa"]
        .apply(lambda x: "-".join(x.unique()))
        .reset_index()
    )
    userx.loc[userx.tarifa.str[-1] == "-", "tarifa"] = userx.loc[
        userx.tarifa.str[-1] == "-", :
    ].tarifa.str[:-1]
    userx.loc[userx.tarifa.str[:1] == "-", "tarifa"] = userx.loc[
        userx.tarifa.str[:1] == "-", :
...
def preparo_etapas_agregadas(etapas, viajes):
    ...
    e_agg = e_agg.groupby(
        ["mes", "tipo_dia", "h3_o", "h3_d", "modo", "id_linea"], as_index=False
    ).factor_expansion_linea.mean()
    ...
    v_agg = v_agg.groupby(
        ["mes", "tipo_dia", "h3_o", "h3_d", "modo"], as_index=False
    ).factor_expansion_linea.mean()
    v_agg = v_agg[v_agg.h3_o != v_agg.h3_d]

    etapas["etapas_max"] = etapas.groupby(
        ["dia", "id_tarjeta", "id_viaje"]
    ...
    transfers = transfers.groupby(
        ["mes", "tipo_dia", "h3_o", "h3_d", "modo", "seq_lineas"], as_index=False
    ).factor_expansion_linea.mean()
...
def preparo_lineas_deseo(
    etapas_selec, viajes_selec, polygons_h3="", poligonos="", res=6
):
    ...
    for id_polygon in polygons_h3.id_polygon.unique():
        ...
        if len(etapas_all[etapas_all.etapa_max > 3]) > 0:
            nborrar = (
                len(
                    etapas_all[etapas_all.etapa_max > 3][
                        ["id_tarjeta", "id_viaje"]
                    ].value_counts()
                )
        ...
        for zona in zonas:
            ...
            for i in ["inicio_norm", "transfer1_norm", "transfer2_norm", "fin_norm"]:
                ...
                if "res_" in zona:
                    ...
                    zonas_data_ = zonas_data.groupby(
                        ["h3", "fex", "latitud", "longitud"], as_index=False
                    )[zona].first()
                    etapas_agrupadas_zon = etapas_agrupadas_zon.merge(
                        zonas_data_[["h3", zona]].rename(
            ...
            etapas_agrupadas_zon = normalizo_zona(
                etapas_agrupadas_zon, zonificaciones[zonificaciones.zona == zona].copy()
            )
            ...
            sum_viajes = (
                ...
                .factor_expansion_linea.sum()
                .groupby(["mes", "tipo_dia", "zona"], as_index=False)
                .factor_expansion_linea.mean()
                .round()
            )
            ...
            sum_viajes["factor_expansion_linea"] = 1 - (
                ...
                / etapas_agrupadas_zon.groupby(
                    ["mes", "tipo_dia", "zona"], as_index=False
                )
                .factor_expansion_linea.sum()
                .factor_expansion_linea
            )
            ...
            sum_viajes["factor_expansion_linea"] = 1 - (
                sum_viajes["factor_expansion_linea"]
                / viajes_matrices.groupby(["mes", "tipo_dia", "zona"], as_index=False)
                .factor_expansion_linea.sum()
                .factor_expansion_linea
            )
            sum_viajes = sum_viajes.rename(
...
def guarda_particion_modal(etapas):
    ...
    etapas_modos = (
        etapas.groupby(
            ["mes", "tipo_dia", "genero", "id_tarjeta", "id_viaje"], as_index=False
        )
        .factor_expansion_linea.mean()
        .merge(
            etapas.groupby(["dia", "id_tarjeta", "id_viaje"], as_index=False)[
    ...
    guardar_tabla_sql(
        ...
        filtros={"mes": etapas_modos.mes.unique().tolist()},
    )
...
def resumen_x_linea(etapas, viajes):
    ...
    all = all.groupby(
        ["mes", "id_linea", "nombre_linea", "empresa", "modo"], as_index=False
    )[
        [
            "transacciones",
            "distancia_media",
    ...
    guardar_tabla_sql(all, "resumen_lineas", "dash", {"mes": all.mes.unique().tolist()})
    ...
    all = all.groupby(
        ["mes", "id_linea", "id_ramal", "nombre_linea", "empresa", "modo"],
        as_index=False,
    )[
        [
            "transacciones",
    ...
    guardar_tabla_sql(
        all, "resumen_lineas_ramal", "dash", {"mes": all.mes.unique().tolist()}
    )
...
def proceso_poligonos(check_configs=True):
    ...
    if len(poligonos) > 0:
        ...
        preparo_lineas_deseo(
            etapas_selec, viajes_selec, polygons_h3, poligonos=poligonos, res=[6, 7]
        )
