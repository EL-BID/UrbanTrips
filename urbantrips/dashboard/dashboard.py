...
if len(indicadores) > 0:
    ...
    trx = indicadores.loc[indicadores.tabla == "transacciones", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Transacciones')
    col2.write(trx)
    trx = indicadores.loc[indicadores.tabla == "etapas", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Etapas')
    col2.write(trx)    
    trx = indicadores.loc[indicadores.tabla == "viajes", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Viajes')
    col2.write(trx)
    trx = indicadores.loc[indicadores.tabla.isin([ "tarjetas", "usuarios"]), ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Tarjetas')
    col2.write(trx)
    trx = indicadores.loc[indicadores.tabla == "etapas_expandidas", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Etapas expandidas')
    col2.write(trx)
    trx = indicadores.loc[indicadores.tabla == "viajes expandidos", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Etapas expandidas')
    col2.write(trx)    
    trx = indicadores.loc[indicadores.tabla == "usuarios expandidos", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Etapas expandidas')
    col2.write(trx) 
    trx = indicadores.loc[indicadores.tabla == "modos viajes", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Partición modal Viajes')
    col2.write(trx) 
    trx = indicadores.loc[indicadores.tabla == "avg", ["detalle", 'indicador', 'porcentaje']].copy()
    col2.write('Promedios')
    col2.write(trx) 
#     titulo = indicadores.loc[indicadores.nivel == 1].Titulo.unique()[0]
