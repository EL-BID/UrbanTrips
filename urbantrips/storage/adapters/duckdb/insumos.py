...
class DuckDBInsumoAdapter:
    ...
    def get_routes(self) -> gpd.GeoDataFrame:
        try:
            with self._conn() as conn:
                df = conn.execute("SELECT * FROM lines_geoms").fetchdf()
        except Exception:
    ...
    def get_stops(self) -> pd.DataFrame:
        with self._conn() as conn:
            return conn.execute("SELECT * FROM stops").fetchdf()
    ...
    def get_metadata_lineas(self) -> pd.DataFrame:
        with self._conn() as conn:
            try:
                return conn.execute("SELECT * FROM metadata_lineas").fetchdf()
            except Exception:
                return pd.DataFrame()
    ...
    def get_metadata_ramales(self) -> pd.DataFrame:
        with self._conn() as conn:
            try:
                return conn.execute("SELECT * FROM metadata_ramales").fetchdf()
            except Exception:
                return pd.DataFrame()

    def get_matrix_validation(self) -> pd.DataFrame:
        with self._conn() as conn:
            return conn.execute("SELECT * FROM matriz_validacion").fetchdf()

    def get_travel_times_stations(self) -> pd.DataFrame:
        with self._conn() as conn:
            try:
                return conn.execute("SELECT * FROM travel_times_stations").fetchdf()
            except Exception:
                return pd.DataFrame()
