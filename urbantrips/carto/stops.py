...
def interpolate_stops_every_x_meters(gdf):
    ...
    for i, row in gdf.iterrows():
        ...
        line_stops_data = create_stops_from_route_geom(
            route_geom=route_geom, stops_distance=stops_distance
        )
...
def aggregate_line_stops_to_node_id(stops_gdf):
    ...
    stops_df = (
        stops_gdf.groupby("id_linea", as_index=False)
...
def create_stops_from_route_geom(route_geom, stops_distance):
    ...
    stops_df = pd.DataFrame(range(len(stop_points)), columns=["branch_stop_order"])
    stops_df = gpd.GeoDataFrame(stops_df, geometry=stop_points, crs=f"EPSG:{epsg_m}")
...
def create_node_id(line_stops_gdf):
    ...
    connectivity = libpysal.weights.fuzzy_contiguity(
        gdf=gdf, buffering=True, drop=False, buffer=buffer, predicate="intersects"
    )
    ...
    x_new_long = gdf.groupby("node_id").apply(lambda df: df.stop_x.mean()).to_dict()
    y_new_long = gdf.groupby("node_id").apply(lambda df: df.stop_y.mean()).to_dict()
