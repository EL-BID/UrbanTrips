...
def compute_lines_od_matrix(
    ...
    if (len(route_geoms) > 0) and (len(legs) > 0):
        ...
        line_od_matrix = line_od_matrix.droplevel(2, axis=0).reset_index()
...
def compute_line_od_matrix(df, route_geoms, hour_range, day_type, save_csv=False):
    ...
    if (route_geoms.id_linea == line_id).any():
        ...
        n_sections = route_geoms.loc[
            route_geoms.id_linea == line_id, "n_sections"
        ].item()

        route_geom = route_geoms.loc[route_geoms.id_linea == line_id, "geometry"].item()
        ...
        totals_by_day = df.groupby(["dia"], as_index=False).agg(
            daily_legs=("factor_expansion_linea", "sum")
        )
        ...
        df["o_proj"] = pd.cut(df.o_proj, bins=section_ids, labels=labels, right=True)
        df["d_proj"] = pd.cut(df.d_proj, bins=section_ids, labels=labels, right=True)

        totals_by_day_section_id = (
            df.groupby(["dia", "o_proj", "d_proj"])
        ...
        totals_by_typeday_section_id = totals_by_typeday_section_id.rename(
            columns={"o_proj": "section_id_o", "d_proj": "section_id_d"}
        )
